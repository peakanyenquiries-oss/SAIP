...
import { Car, Database, Factory, Search, PackageCheck, Wrench, CheckCircle2, AlertTriangle, ShoppingCart } from "lucide-react";
      ...
      {selectedVehicleId && (
        <>
          <SectionCard title="Service Kit Intelligence" subtitle={selectedVehicle ? `${selectedVehicle.make} ${selectedVehicle.model} ${selectedVehicle.variant} · ${selectedVehicle.engineCode || "Engine not specified"}` : "Selected vehicle variant"}>
            ...
            {!kitLoading && selectedKitId && !recommendation && !kitError && serviceKits.length === 0 && <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-800">No active service kits are currently configured in the live master data.</div>}
            ...
            {recommendation && !kitLoading && (
              <div className="mt-6 space-y-6">
                ...
                <div className="overflow-x-auto rounded-xl border border-slate-200">
                  <table className="min-w-full text-sm"><thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Product</th><th className="px-4 py-3">Qty</th><th className="px-4 py-3">Fitment</th><th className="px-4 py-3">Supplier</th><th className="px-4 py-3">Stock</th><th className="px-4 py-3">Cost</th><th className="px-4 py-3">Selling</th><th className="px-4 py-3">Procurement</th><th className="px-4 py-3">Status</th></tr></thead><tbody className="divide-y divide-slate-100 bg-white">{recommendation.items.map((item) => <tr key={item.productId}><td className="px-4 py-3 font-medium">{item.productName}{item.sku ? ` · ${item.sku}` : ""}</td><td className="px-4 py-3">{item.quantity}{item.required ? " · Required" : " · Optional"}</td><td className="px-4 py-3">{item.compatible ? <span className="text-emerald-700">{item.fitmentType}</span> : <span className="text-red-600">Not confirmed</span>}</td><td className="px-4 py-3">{item.supplierName || "Unassigned"}</td><td className="px-4 py-3">{item.stock}</td><td className="px-4 py-3">R {item.costPrice.toFixed(2)}</td><td className="px-4 py-3">R {item.sellingPrice.toFixed(2)}</td><td className="px-4 py-3">{item.procurement ? <div className="space-y-1"><div className={`font-semibold ${item.procurement.recommendation === "ORDER_NOW" ? "text-red-700" : item.procurement.recommendation === "EXPEDITE_REVIEW" ? "text-amber-700" : "text-slate-700"}`}>{item.procurement.recommendation.replaceAll("_", " ")}</div><div className="text-xs text-slate-500">{item.procurement.supplierName || "No supplier"} · Score {item.procurement.supplierScore.toFixed(1)} · Qty {item.procurement.recommendedQuantity}</div><div className="text-xs text-slate-500">Lead {item.procurement.leadTimeDays}d · R {item.procurement.orderValue.toFixed(2)}</div></div> : <span className="text-slate-400">No recommendation</span>}</td><td className="px-4 py-3">{item.compatible && item.stock >= item.quantity ? <span className="font-semibold text-emerald-700">Ready</span> : <span className="font-semibold text-amber-700">Attention</span>}</td></tr>)}</tbody></table>
                </div>
              </div>
            )}
          </SectionCard>

          <SectionCard title="Compatible Products" subtitle="Live fitment, supplier and procurement intelligence for this vehicle variant.">
