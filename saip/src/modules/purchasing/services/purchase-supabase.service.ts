...
export async function addPurchaseOrder(
  ...
  const saved =
    await getPurchaseOrderById(data.id);

  if (!saved) {
    throw new Error(
      "Purchase order was created but could not be loaded."
    );
  }
...
export async function updatePurchaseOrder(
  ...
  const saved =
    await getPurchaseOrderById(order.id);

  if (!saved) {
    throw new Error(
      "Purchase order was updated but could not be loaded."
    );
  }
...
export async function receivePurchaseOrder(
  ...
  for (const [itemId, quantity] of Object.entries(
    ...
    cleanQuantities[itemId] =
      Math.floor(numericQuantity);
  }

  if (
    Object.keys(cleanQuantities).length === 0
  ) {
    return {
      success: false,
      message:
        "No valid receiving quantities were supplied.",
    };
  }
